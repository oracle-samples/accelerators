 ...
 *  SHA1: $Id: 52108cda84b8dba41d55dc3c4687e0af6b43526b $
 * *********************************************************************************************
 *  File: logic.js
 * ****************************************************************************************** */

RightNow.namespace('Custom.Widgets.EbsServiceRequest.PostalValidate');
Custom.Widgets.EbsServiceRequest.PostalValidate = RightNow.Widgets.TextInput.extend({ 
    /**
    ...
    overrides: {
        ...
        constructor: function() {
            ...
            */
            ...
            console.log('idZip we\'ll be getting: ' + this.idZip); // Cannot assume Zip and current WidgetField are the same.
    ...
    _onVerifyButtonClick: function(evt) {
        ...
        this._state = typeof list.options !== 'undefined' ? list.options[list.selectedIndex].text : list.value;
                
        // used only for checking!;
        list = document.getElementById(this.idCountry);
        if (this.idCountry && list) {
            this._country = typeof list.options !== 'undefined' ? list.options[list.selectedIndex].text : list.value;
        } 
    ...
    _onValidateUseItClick: function(evt) {
        ...
        if (checked) {
            ...
            var stateElem = document.getElementById(this.idState);
            if (typeof stateElem.options !== 'undefined') {
                // is a drop-down list in this case.
                var optslen = stateElem.options.length;
                for (var i = 0; i < optslen; i++) {
                    if (stateElem.options[i].text == this._veroAddress.STATE){
                        stateElem.selectedIndex = i;    // dunno why doing both 
                        stateElem.options[i].selected = true;                
                        break;
                    } else {
                        stateElem.options[i].selected = false;
                    }
                }
            } else {
                // if it's a custom field and not a list. 
                stateElem.value = this._veroAddress.STATE;
            }
        }
    },
