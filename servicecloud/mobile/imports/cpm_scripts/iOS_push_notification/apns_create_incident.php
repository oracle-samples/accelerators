<?
/*
 * CPMObjectEventHandler: apns_create_incident
 * Package: RN
 * Objects: Incident
 * Actions: Create
 * Version: 1.3
...
class apns_create_incident implements RNCPM\ObjectEventHandler
{
    ...
    public static function apply($run_mode, $action, $obj, $n_cycles)
    {
        ...
        $severity_id     = $obj->Severity->ID;
        $fetchedToken = $acct->CustomFields->Mobile->apns_token;

        if (!empty($fetchedToken) && $severity_id == 1) {
...
// class apns_create_incident


/*
The Test Harness
*/
class apns_create_incident_TestHarness implements RNCPM\ObjectEventHandler_TestHarness
{
    ...
    public static function setup()
    {
        ...
        // For this test, create a new incident as expected.

        $inc = new RNCPHP\Incident;

        $inc->Subject = "New Incident";

        
        $inc->AssignedTo->Account = RNCPHP\Account::fetch(1);
        $inc->PrimaryContact = RNCPHP\Contact::fetch(1);
